...
import org.example.samlraider.helpers.XMLHelpers;
...
public class SamlTabController /*implements IMessageEditorTab, Observer*/ {
    ...
    */
    ...
    //                xmlHelpers.signMessage(document, signAlgorithm, digestAlgorithm,
    // cert.getCertificate(),
    //                        cert.getPrivateKey());
    ...
    //                    String HTTPHeader = new String(byteMessage, 0, bodyOffset,
    // StandardCharsets.UTF_8);
    //
    //                    String soapMessage = new String(byteMessage, bodyOffset,
    // byteMessage.length - bodyOffset, StandardCharsets.UTF_8);
    //                    Document soapDocument =
    // xmlHelpers.getXMLDocumentOfSAMLMessage(soapMessage);
    //                    Element soapBody = xmlHelpers.getSOAPBody(soapDocument);
    //                    xmlHelpers.getString(soapDocument);
    //                    Document samlDocumentEdited =
    // xmlHelpers.getXMLDocumentOfSAMLMessage(SAMLMessage);
    ...
    //
    // .getStringOfDocument(xmlHelpers.getXMLDocumentOfSAMLMessage(new String(textArea.getText())),
    // 0, true);
    //                    } catch (IOException e) {
    //                        setInfoMessageText(XML_COULD_NOT_SERIALIZE);
    //                    } catch (SAXException e) {
    ...
    //                }
    //                IParameter newParameter = helpers.buildParameter(parameterToUpdate,
    // getEncodedSAMLMessage(textMessage),
    //                        parameterType);
    //
    ...
    //                String soapMessage = new String(content, bodyOffset, content.length -
    // bodyOffset, StandardCharsets.UTF_8);
    ...
    //            try {
    //                IRequestInfo requestInfo = helpers.analyzeRequest(content);
    //                isWSSUrlEncoded = requestInfo.getContentType() ==
    ...
    //            requestParameter = helpers.getRequestParameter(content,
    // certificateTabController.getSamlResponseParameterName());
    ...
    //
    //                    if (isSAMLRequest) {
    //                        parameter = helpers.getRequestParameter(content,
    // certificateTabController.getSamlRequestParameterName());
    //                    } else {
    //                        parameter = helpers.getRequestParameter(content,
    // certificateTabController.getSamlResponseParameterName());
    ...
    //        resetInfoMessageText();
    //        try {
    //            Document document = xmlHelpers.getXMLDocumentOfSAMLMessage(new
    ...
    //                Document doc = xmlHelpers.getXMLDocumentOfSAMLMessage(string);
    //                xmlHelpers.removeEmptyTags(doc);
    //                xmlHelpers.signAssertion(doc, signAlgorithm, digestAlgorithm,
    // cert.getCertificate(),
    //                        cert.getPrivateKey());
    ...
    //            resetInfoMessageText();
    //            if (isWSSMessage) {
    //                setInfoMessageText("Message signing is not possible with WS-Security
    // messages");
    //            } else {
    //                setInfoMessageText("Signing...");
    //                BurpCertificate cert = samlGUI.getActionPanel().getSelectedCertificate();
    ...
    // xmlHelpers.getSignatureAlgorithm(responses.item(0));
    //                    String digestAlgorithm = xmlHelpers.getDigestAlgorithm(responses.item(0));
    //
    //                    xmlHelpers.removeOnlyMessageSignature(document);
    //                    xmlHelpers.signMessage(document, signAlgorithm, digestAlgorithm,
    ...
    //        String xxePayload = "<!DOCTYPE foo [ <!ENTITY % xxe SYSTEM \"" + collabUrl + "\">
    // %xxe; ]>\n";
