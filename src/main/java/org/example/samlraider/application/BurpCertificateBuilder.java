 ...
 */
package org.example.samlraider.application;
...
import org.example.samlraider.helpers.CertificateHelper;
import org.example.samlraider.model.BurpCertificate;
import org.example.samlraider.model.BurpCertificateExtension;
...
public class BurpCertificateBuilder {
     ...
     * The certificate is signed with the private key of the given issuer.
     *
     * @param issuer The Private Key of this issuer is used for signing
     ...
     */
    public BurpCertificate generateCertificate(BurpCertificate issuer)
            throws CertificateEncodingException,
                    InvalidKeyException,
    ...
    /**
     * Generates a new certificate and sets the fields Private/Public Key and Source of this object.
     * The certificate is signed with the private key of "this" object.
     *
     ...
     * @throws IOException
     */
    public BurpCertificate generateSelfSignedCertificate()
            throws CertificateEncodingException,
                    InvalidKeyException,
                    IllegalStateException,
                    NoSuchAlgorithmException,
     ...
     *
     * @param privateKey which signes the certificates
     ...
     * @throws IOException
     */
    private X509Certificate generateX509Certificate(PrivateKey privateKey)
            throws CertificateEncodingException,
                    InvalidKeyException,
                    IllegalStateException,
                    NoSuchAlgorithmException,
        ...
        if (hasBasicConstraints) {
            if (isCA && hasNoPathLimit) {
                ...
                certificateGenerator.addExtension(
                        X509Extensions.BasicConstraints, true, new BasicConstraints(pathLimit));
            } else {
                certificateGenerator.addExtension(
                        X509Extensions.BasicConstraints, true, new BasicConstraints(false));
            }
        }
        ...
        if (keyUsage.size() > 0) {
            ...
            certificateGenerator.addExtension(
                    X509Extensions.KeyUsage, true, new KeyUsage(allKeyUsages));
        }
        ...
        if (extendedKeyUsage.size() > 0) {
            ...
            certificateGenerator.addExtension(
                    X509Extensions.ExtendedKeyUsage, false, new DERSequence(allExtendedKeyUsages));
        }

        if (subjectAlternativeName.size() > 0) {
            GeneralNames generalNames =
                    new GeneralNames(
                            subjectAlternativeName.toArray(
                                    new GeneralName[subjectAlternativeName.size()]));
            certificateGenerator.addExtension(
                    X509Extensions.SubjectAlternativeName, true, generalNames);
        }
        ...
        if (issuerAlternativeName.size() > 0) {
            GeneralNames generalNames =
                    new GeneralNames(
                            issuerAlternativeName.toArray(
                                    new GeneralName[issuerAlternativeName.size()]));
            certificateGenerator.addExtension(
                    X509Extensions.IssuerAlternativeName, true, generalNames);
        }
        ...
        if (setAuthorityKeyIdentifier == true && issuerCertificate != null) {
            ...
            certificateGenerator.addExtension(
                    ...
                    true,
                    j.createAuthorityKeyIdentifier(issuerCertificate));
        }
        ...
        for (BurpCertificateExtension e : burpCertificateExtensions) {
            ...
            ASN1Encodable extension =
                    X509ExtensionUtil.fromExtensionValue(e.getExtensionValue()); // Finally!!!
    ...
    /**
     * Generates a Public and Private Key with the minimum size of 512 Bytes and set the variables
     * of this object.
     *
     * @throws NoSuchAlgorithmException
     * @throws NoSuchProviderException
    ...
    public void addSubjectAlternativeName(String type, String name) {
        subjectAlternativeName.add(
                new GeneralName(ObjectIdentifier.getX509SubjectAlternativeNames(type), name));
    }
