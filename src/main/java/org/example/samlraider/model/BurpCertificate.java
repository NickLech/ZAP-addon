 ...
 */
package org.example.samlraider.model;
...
public class BurpCertificate {
    ...
    public boolean hasExtensions() {
        return (!certificate.getCriticalExtensionOIDs().isEmpty())
                || (!certificate.getNonCriticalExtensionOIDs().isEmpty());
    }
    ...
    public String getSignature() {
        return CertificateHelper.addHexColons(
                CertificateHelper.byteArrayToHex(certificate.getSignature()));
    }
        ...
        try {
            ...
            for (List<?> i : certificate.getSubjectAlternativeNames()) {
                subjectAlternativeNames.add(
                        i.get(1)
                                + " ("
                                + ObjectIdentifier.getSubjectAlternativeNames((Integer) i.get(0))
                                + ")");
            }
    ...
    public String getBasicConstraints() {
        ...
        switch (certificate.getBasicConstraints()) {
            ...
            default:
                basicConstraints =
                        "CA: True. Path limit: " + certificate.getBasicConstraints() + ".";
        }
    ...
    public List<BurpCertificateExtension> getAllExtensions() {
        ...
        if (criticalExtensionOIDs != null) {
            for (String i : criticalExtensionOIDs) {
                allExtensions.add(
                        new BurpCertificateExtension(i, true, certificate.getExtensionValue(i)));
            }
        }
        ...
        if (nonCriticalExtensionOIDs != null) {
            for (String i : nonCriticalExtensionOIDs) {
                allExtensions.add(
                        new BurpCertificateExtension(i, false, certificate.getExtensionValue(i)));
            }
        }
